...
String.prototype.trim = function() {
    ...
    return this.replace(new RegExp(e.replace(/([\/\,\!\\\^\$\{\}\[\]\(\)\.\*\+\?\|\<\>\-\&])/g, "\\####INJECT####"), s ? "gi" : "g"), "string" == typeof r ? r.replace(/\$/g, "$$") : r)
};
var shader_global = function() {
    ...
    eash = {
        ...
        linerPostProcess: function(e, r, s) {
            ...
            eash.ind++;
            var a = new BABYLON.PostProcess("name" + eash.ind, t, ["camera", "mouse", "time", "screen", "glb", "center"], ["ref1", "ref2", "ref3", "ref4", "ref5", "ref6", "ref7"], def(s.scale, 1), r, BABYLON.Texture.BILINEAR_SAMPLINGMODE);
            return a.onApply = function(e) {
                e.setFloat("time", time), e.setVector2("screen", {
                    x: a.width,
                    y: a.height
                }), e.setVector3("camera", r.position), def(s.apply) && s.apply(e)
            }, a
        }
    };
eash.fback = "!gl_FrontFacing", eash.ffront = "gl_FrontFacing", eash.discard = "discard", shcolor = function(e, r, s, t) {
    var a = {
        r: 0,
        g: 0,
        b: 0,
...
ColorPs = {
    ...
    for (var s = eash.ind++, t = "", a = ["", "", "", ""], n = 0, c = 0; c < e.length; c++) def(e[c].r) || (e[c] = {
        r: e[c],
        e: 1
    }), t += " vec4 result_" + s + "_" + c + ";result_" + s + "_" + c + " = vec4(0.,0.,0.,0.); float rp_" + s + "_" + c + " = " + _cs(e[c].e) + "; \n", t += e[c].r + "\n", t += " result_" + s + "_" + c + " = result; \n", a[0] += (0 == c ? "" : " + ") + "result_" + s + "_" + c + ".x*rp_" + s + "_" + c, a[1] += (0 == c ? "" : " + ") + "result_" + s + "_" + c + ".y*rp_" + s + "_" + c, a[2] += (0 == c ? "" : " + ") + "result_" + s + "_" + c + ".z*rp_" + s + "_" + c, a[3] += (0 == c ? "" : " + ") + "result_" + s + "_" + c + ".w*rp_" + s + "_" + c, n += e[c].e;
...
}, eash.reference = function(e, r) {
    if (e = def(e, "ref"), r = def(r, ""), "ref" != e) {
        {
            eash.ind++
        }
        return "vec4 res_" + e + "_ = result; " + r + " ref = result; result = res_" + e + "_; "
    }
    return r + " ref = result; "
}, eash.replace = function(e) {
    var r = eash.ind++;
    e = def(e, {}), e.type = def(e.type, red), e.mat = def(e.mat, eash.solid(16711935)), e.area = def(e.area, -.233), e.opacity = def(e.opacity, 0), e.level = def(e.level, 0), e.levelCount = def(e.levelCount, 1), e.levelFill = def(e.levelFill, !1), e.live = def(e.live, !1), e.live && eash.reference();
    var s = e.area,
        t = e.opacity,
        a = e.level,
        n = e.levelCount,
        ...
        o = " > 0.5 + " + _cs(s) + " ",
        i = " < 0.5 - " + _cs(s) + " ",
        l = "((ref.x*" + _cs(n) + "-" + _cs(a) + ")>1.0 ? 0. : max(0.,(ref.x*" + _cs(n) + "-" + _cs(a) + ")))",
        f = "((ref.y*" + _cs(n) + "-" + _cs(a) + ")>1.0 ? 0. : max(0.,(ref.y*" + _cs(n) + "-" + _cs(a) + ")))",
        _ = "((ref.z*" + _cs(n) + "-" + _cs(a) + ")>1.0 ? 0. : max(0.,(ref.z*" + _cs(n) + "-" + _cs(a) + ")))";
    c && (l = "min(1.0, max(0.,(ref.x*" + _cs(n) + "-" + _cs(a) + ")))", f = "min(1.0, max(0.,(ref.y*" + _cs(n) + "-" + _cs(a) + ")))", _ = "min(1.0, max(0.,(ref.z*" + _cs(n) + "-" + _cs(a) + ")))");
    var h = " && ",
        u = " + ",
        d = "",
        p = "";
    switch (e.type) {
        case white:
            p = l + o + h + f + o + h + _ + o, d = "(" + l + u + f + u + _ + ")/3.0";
            break;
        case cyan:
            p = l + i + h + f + o + h + _ + o, d = "(" + f + u + _ + ")/2.0 - (" + l + ")/1.0";
            break;
        case pink:
            p = l + o + h + f + i + h + _ + o, d = "(" + l + u + _ + ")/2.0 - (" + f + ")/1.0";
            break;
        case yellow:
            p = l + o + h + f + o + h + _ + i, d = "(" + l + u + f + ")/2.0 - (" + _ + ")/1.0";
            break;
        case blue:
            p = l + i + h + f + i + h + _ + o, d = "(" + _ + ")/1.0 - (" + l + u + f + ")/2.0";
            break;
        case red:
            p = l + o + h + f + i + h + _ + i, d = "(" + l + ")/1.0 - (" + f + u + _ + ")/2.0";
            break;
        case green:
            p = l + i + h + f + o + h + _ + i, d = "(" + f + ")/1.0 - (" + l + u + _ + ")/2.0";
            break;
        case black:
            p = l + i + h + f + i + h + _ + i, d = "1.0-(" + l + u + f + u + _ + ")/3.0"
    }
    return " if( " + p + " ) { vec4 oldrs_" + r + "_ = vec4(result);float al_" + r + "_ = max(0.0,min(1.0," + d + "+(" + _cs(t) + "))); float  l_" + r + "_ =  1.0-al_" + r + "_;  " + e.mat + " result = result*al_" + r + "_ +  oldrs_" + r + "_ * l_" + r + "_;    }"
}, eash.vertex = function(e) {
    return eash.globalOption = def(eash.globalOption, {}), eash.globalOption.vtx = e, ""
}, eash.cameraShot = function(e) {
    return e = def(e, {}), "result = vec4(texture2D(textureSampler, " + def(e.uv, "uv") + ").xyz,1.0) ;"
}, eash.cameraLayer = function(e, r) {
    return r = def(r, {}), "result = vec4(texture2D(ref" + e + ", " + def(r.uv, "uv") + ").xyz,1.0) ;"
}, eash.filters = {
...
}, eash.glowingEdges = function(e) {
    return e = def(e, {}), eash.cameraShot({
        uv: "uv+ vec2(1.,0.)*0.0015  "
    }) + "vec4 rs1 = result;" + eash.cameraShot({
        uv: "uv+ vec2(1.,1.)*0.0015  "
    }) + "vec4 rs2 = result;" + eash.cameraShot({
        uv: "uv+ vec2(1.,0.)*0.0015  "
    }) + "vec4 rs3 = result;" + eash.cameraShot({
        uv: "uv+ vec2(-1.,-1.)*0.0015  "
    }) + "vec4 rs4 = result;" + eash.cameraShot({
        uv: "uv+ vec2(-1.,0.)*0.0015  "
    }) + "vec4 rs5 = result;" + eash.cameraShot({
        uv: "uv+ vec2(0.,-1.)*0.0015  "
    }) + "vec4 rs6 = result;" + eash.multi([" result = vec4(vec3(pow(length(rs1.xyz-rs2.xyz),1.)),1.0) ;", " result = vec4(vec3(pow(length(rs2.xyz-rs3.xyz),1.)),1.0) ;", " result = vec4(vec3(pow(length(rs3.xyz-rs4.xyz),1.)),1.0) ;", " result = vec4(vec3(pow(length(rs3.xyz-rs4.xyz),1.)),1.0) ;", " result = vec4(vec3(pow(length(rs4.xyz-rs5.xyz),1.)),1.0) ;", " result = vec4(vec3(pow(length(rs5.xyz-rs6.xyz),1.)),1.0) ;", " result = vec4(vec3(pow(length(rs6.xyz-rs1.xyz),1.)),1.0) ;"], !0) + "vec4 rs7 = result; float s = max(0.,min(1.,pow(length(rs7.xyz),0.8)/0.5));" + eash.cameraShot({
        uv: "uv"
    }) + eash.effect({
        pr: "pow(pr,2.0)/0.5"
    }) + " result = vec4(result.xyz*( s),1.0);"
}, eash.ppsMap = function(e, r) {
    return 0 == e ? eash.cameraShot(r) : eash.cameraLayer(e, r)
}, eash.directionBlur = function(e) {
    e = def(e, {}), e.mapInd = def(e.mapInd, 0), e.dir = def(e.dir, {
        x: 1,
        y: 0
    });
    var r = function(r) {
        return 0 == e.mapInd ? eash.cameraShot({
            uv: r
        }) : eash.cameraLayer(e.mapInd, {
            uv: r
        })
    };
    e.percent = def(e.percent, 10), e.h1 = def(e.h1, 1), e.h2 = def(e.h2, 3);
    for (var s = [], t = 1, a = e.h1 * e.percent / 1e4; a < e.percent / 100; a += e.h2 * e.percent / 1e4) {
        t -= .1, t = Math.max(0, t);
        var n = Math.pow(a, .9) / 1.33;
        s.push({
            r: def(e.custom) ? e.custom("vec2(" + _cs(e.dir.x) + "," + _cs(e.dir.y) + ")*" + _cs(n), n, t) : r(" uv  +vec2(" + _cs(e.dir.x) + "," + _cs(e.dir.y) + ")*" + _cs(n)),
            e: t
        }), s.push({
            r: def(e.custom) ? e.custom("-1.*vec2(" + _cs(e.dir.x) + "," + _cs(e.dir.y) + ")*" + _cs(n), n, t) : r(" uv  -vec2(" + _cs(e.dir.x) + "," + _cs(e.dir.y) + ")*" + _cs(n)),
            e: t
        })
    }
    return eash.multi(s, !0)
};